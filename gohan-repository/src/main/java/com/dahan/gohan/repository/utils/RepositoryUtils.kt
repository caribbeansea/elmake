...
object RepositoryUtils {
    ...
    private val repositorys: Map<String, GohanRepository> = Maps.newHashMap( // 默认使用阿里云中央仓库
            "funault", GohanRepository("alibaba-central", "https://maven.aliyun.com/repository/central"))
     ...
     * @return 默认工件储存库
     */
    fun getfunaultRepository(): GohanRepository? = repositorys.get("funault")

    fun resolveDependencies(gohanDependency: GohanDependency): DependencyResult? =
            resolveDependencies(Lists.of(gohanDependency))

    /**
     * 下载所需依赖
     */
    fun resolveDependencies(gohanDependencies: List<GohanDependency>): DependencyResult? {
        val dependencyRequest =
                RequestUtils.newDependencyRequest(gohanDependencies, Lists.newArrayList(repositorys.values), funaultRepository)
