...
object RepositoryUtils {
    ...
    private val REPOSITORYS: Map<String, EimotoRepository> = Maps.newHashMap( // 默认使用阿里云中央仓库
            "funault", EimotoRepository("alibaba-central", "https://maven.aliyun.com/repository/central"))
     ...
     * @return 默认工件储存库
     */
    fun getfunaultRepository(): EimotoRepository? = REPOSITORYS.get("funault")

    fun resolveDependencies(eimotoDependency: EimotoDependency): DependencyResult? =
            resolveDependencies(Lists.of(eimotoDependency))

    /**
     * 下载所需依赖
     */
    fun resolveDependencies(eimotoDependencies: List<EimotoDependency>): DependencyResult? {
        val dependencyRequest =
                RequestUtils.newDependencyRequest(eimotoDependencies, Lists.newArrayList(REPOSITORYS.values), funaultRepository)
